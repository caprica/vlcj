 ...
 * <p>
 * Overlay contents may be implemented by using Swing/AWT components in a
 * layout using {@link #onCreateOverlay()}, and/or by implementing custom 
 * painting using {@link #onNewSize(int, int)} and {@link #onPaintOverlay(Graphics2D)}. 
 * <p>
 * Various template method are provided for sub-classes to provide the required
 * behaviour.
 * <p>
 * The default implementation of the {@link #onSetWindowTransparency()}
 * template method attempts to make a transparent overlay by either the 
 * preferred method of simply setting the background colour to a transparent
 * colour for Java7, or by using {@link AWTUtilities}, if it exists, for Java6.
 */
public abstract class AbstractJWindowOverlayComponent extends JWindow {
   ...
   * Create an overlay.
   * <p>
   * An "alpha-compatible" graphics configuration will be used when creating the
   * window so as to enable a transparent overlay.
   * 
   ...
   * <p>
   * Implementing classes may override this method to, for example, set a 
   * layout and add components to the overlay.
   * <p>
   * Implementing classes may opt to create the overlay in the constructor 
   * instead. 
   */
  ...
  /**
   * Template method used to configure the graphics context before painting the
   * overlay.
   * <p>
